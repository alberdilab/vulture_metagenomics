...
results_adonis[[paste(var, i, sep = "_")]] <- ad
    ...
    plot_title  <- paste("Beta Diversity of", var, "controlling", combo_label) %>% 
      str_wrap(width = 45) 
   ...
   filename <- paste0("Figures/SubsettedData/BetaDiv/", "BetaDiv_", var, "_", 
...
vars <- c("species", "sample_type", "state")
...
for (var in vars) {
  ...
  for (i in seq_len(nrow(combos))) {
    ...
    ad_df <- broom::tidy(ad) %>%
      mutate(variable = var,
             fixed = paste(paste0(combo), collapse = ", "))
...
nmds_scores <- nmds_scores %>%
  ...
  combo_label <- paste0(combos[i, ], collapse = " and ")
  plot_title  <- paste("GIFT Diversity of", var, "controlling", combo_label) %>% 
    str_wrap(width = 45) 

p <- ggplot(nmds_scores, aes(x = MDS1, y = MDS2, color = !!sym(var))) +
  geom_point(size = 4) +
  stat_ellipse(type = "norm", linetype = 2) + 
      annotate("text", x = Inf, y = -Inf, label = paste0("p = ", signif(ad_df$p.value[1], 3)), hjust = 1.1, vjust = -0.5, size = 4) + 
  ...
  labs(title = str_wrap(plot_title, width = 45),
       color = var)
...
# Bind results
functional_adonis <- bind_rows(results_adonis)
...
uniqueGIFT_db<- unique(GIFT_db2) %>% unite("Function",Function:Element, sep= "_", remove=FALSE)
uniqueGIFT_db$DomainSplit = sub("_.*", "", uniqueGIFT_db$Function)

Domains <- unique(uniqueGIFT_db$DomainSplit)
Domain_colors <- setNames(viridis(length(Domains), option = "plasma"), Domains)
...
for (var in meta_vars) {
  ...
  for (i in seq_len(nrow(fixed_combos))) {
    ...
    sig_wilcox$Element = uniqueGIFT_db$Element[match(sig_wilcox$trait, uniqueGIFT_db$Code_element)]
    sig_wilcox$Domain = uniqueGIFT_db$DomainSplit[match(sig_wilcox$trait, uniqueGIFT_db$Code_element)]
    ...
    if(sum(heatmap_data$p_adjust <= 0.05) > 0){
    ...
    if(sum(sig_wilcox$p_adjust <= 0.05) > 0){
    ...
    labs(
      title = paste("Volcano Plot comparing", var, "with", paste(combo[fixed_vars], collapse = " and "), "fixed"),
      x = "GIFT Difference",
      y = "-Log10(Adjusted P-value)",
      color = "Functional Domain"
...
ggplot(merged_p, aes(x = neglog_p_taxonomic, y = neglog_p_functional)) +
  ...
  theme_minimal() +
  labs(
    title = "Comparison of p-values between Taxonomic and Functional Diversity",
...
ggsave("Figures/SubsettedData/TaxonomicFuncationalCorrelation.png", plot = p, width = 6, height = 5, dpi = 300)

```


<!--chapter:end:06_taxonomic_functional_comparison.Rmd-->
