...
```{r ancom_gift_sampletype, include= FALSE}
...
```{r GIFT_sig_sampletype, fig.height=5, fig.width=10, fig.fullwidth=TRUE, include = FALSE}
...
### Wilcoxon comparison
```{r community_elem, comment="", echo=FALSE, message=FALSE, warning=FALSE}
element_gift <- GIFTs_elements_community_no0 %>% 
  as.data.frame() %>% 
  rownames_to_column(., "sample") %>% 
  #Change column in sample_metadata to change comparison sample_type, species, etc
...
sig_wilcox_sample = element_gift %>%
  ...
  summarise(
    ...
    mean_1 = mean(value[comparison == unique(element_gift[,comparison])[1]], na.rm = TRUE),
    mean_2 = mean(value[comparison == unique(element_gift[,comparison])[2]], na.rm = TRUE),
...
wilcox_elements_community_no0_sig %>%
    as.data.frame() %>%
    rownames_to_column(var="sample") %>%
    pivot_longer(!sample,names_to="trait",values_to="gift") %>%
    left_join(sample_metadata, by = join_by(sample == sample)) %>%
as.data.frame(wilcox_elements_community_no0_sig) %>%
    mutate(functionid = substr(trait, 1, 3)) %>%
    mutate(trait = case_when(
      trait %in% GIFT_db2$Code_element ~ GIFT_db2$Element[match(trait, GIFT_db2$Code_element)],
      TRUE ~ trait
    ...
    ggplot(aes(x=sample,y=trait,fill=gift)) +
        ...
        scale_fill_gradientn(colours=rev(c("#2066a8", "#ededed"))) + 
        #facet_nested(~ species + sample_type,  scales="free", space="free") + 
        facet_grid(functionid ~ get(comparison), scales="free",space="free") +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
              strip.text.y = element_text(angle = 0), axis.text.y = element_text(angle=45, size=5)) + 
        labs(y="Functional Attribute",x="Samples",fill="GIFT")
