...
```
### Taxonomy barplot per individual by host
...
### Taxonomy barplot per individual by sample type
...
``` {r function_div_species_state, warning=FALSE, comments="", message=FALSE, echo=FALSE}
beta_q1f$S %>%
  vegan::metaMDS(., trymax = 500, k = 2, trace=0) %>%
  vegan::scores() %>%
  as_tibble(., rownames = "sample") %>%
...
``` {r function_div_state, warning=FALSE, comments="", message=FALSE, echo=FALSE}
beta_q1f$S %>%
  vegan::metaMDS(., trymax = 500, k = 2, trace=0) %>%
  vegan::scores() %>%
  as_tibble(., rownames = "sample") %>% 
...
adonis2(dist_func ~ sample_metadata_non0$species, 
...
```
###Identify Significant functions by Sample Type
```{r ancom_gift_sampletype}

elements_table_ps <- otu_table(as.matrix(t(GIFTs_elements_community_no0)), taxa_are_rows = TRUE)

# Convert metadata to phyloseq sample_data object
sample_data_ps <- sample_data(column_to_rownames(sample_metadata, "sample"))

# Combine into phyloseq object
ancom_table <- phyloseq(elements_table_ps, sample_data_ps)

exp_sampletype <- ancombc2(
    data = ancom_table,
...
# View results
lfc_sampletype <- as.data.frame(exp_sampletype$res$taxon)
colnames(lfc_sampletype)[1] = "Function"
rownames(lfc_sampletype) = lfc_sampletype$Function
lfc_sampletype$Function = NULL
lfc_sampletype = mutate(lfc_sampletype, lfc = exp_sampletype$res$`lfc_sample_typeStomach contents`)
lfc_sampletype = mutate(lfc_sampletype, q_val = exp_sampletype$res$`q_sample_typeStomach contents`)

# See significant taxa
sig_func_sampletype = filter(lfc_sampletype, q_val < 0.05)

print(sig_func_sampletype)

```


```{r GIFT_sig_sampletype, fig.height=5, fig.width=10, fig.fullwidth=TRUE}
#Match significant functions to table
GIFTs_elements_community_no0_sig = GIFTs_elements_community_no0[, which(match(colnames(GIFTs_elements_community_no0), rownames(sig_func_sampletype))>0)]

GIFTs_elements_community_no0_sig %>%
    as.data.frame() %>%
    rownames_to_column(var="sample") %>%
    pivot_longer(!sample,names_to="trait",values_to="gift") %>%
    left_join(sample_metadata, by = join_by(sample == sample)) %>%
as.data.frame(GIFTs_elements_community_no0_sig) %>%
    mutate(functionid = substr(trait, 1, 3)) %>%
    mutate(trait = case_when(
      trait %in% GIFT_db2$Code_element ~ GIFT_db2$Element[match(trait, GIFT_db2$Code_element)],
      TRUE ~ trait
...
```
###Identify Significant Functions by Host Species
```{r ancom_gift_species, warning=FALSE}
...
# View results
lfc_species <- as.data.frame(exp_species$res$taxon)
colnames(lfc_species)[1] = "Function"
rownames(lfc_species) = lfc_species$Function
lfc_species$Function = NULL

lfc_species = mutate(lfc_species, lfc = exp_species$res$`lfc_speciesCoragyps atratus`)
lfc_species = mutate(lfc_species, q_val = exp_species$res$`q_speciesCoragyps atratus`)

# See significant taxa
sig_func_species = filter(lfc_species, q_val < 0.05)

print(sig_func_species)

```


```{r GIFT_sig_species, fig.height=5, fig.width=10, fig.fullwidth=TRUE}
#Match significant functions to table
GIFTs_elements_community_no0_sig = GIFTs_elements_community_no0[, which(match(colnames(GIFTs_elements_community_no0), rownames(sig_func_species))>0)]
...
```
###Identify Significant Functions by Sex
```{r ancom_gift_sex, warning=FALSE}
...
# View results
lfc_sex <- as.data.frame(exp_sex$res$taxon)
colnames(lfc_sex)[1] = "Function"
rownames(lfc_sex) = lfc_sex$Function
lfc_sex$Function = NULL

lfc_sex = mutate(lfc_sex, lfc = exp_sex$res$`lfc_sexMale`)
lfc_sex = mutate(lfc_sex, q_val = exp_sex$res$`q_sexMale`)

# See significant taxa
sig_func_sex = filter(lfc_sex, q_val < 0.05)
...
```{r GIFT_sig_sex, fig.height=5, fig.width=10, fig.fullwidth=TRUE}
#Match significant functions to table
GIFTs_elements_community_no0_sig = GIFTs_elements_community_no0[, which(match(colnames(GIFTs_elements_community_no0), rownames(sig_func_sex))>0)]
...
###Identify Significant Functions by State
```{r ancom_gift_state, warning=FALSE}
...
# View results
lfc_state <- as.data.frame(exp_state$res$taxon)
colnames(lfc_state)[1] = "Function"
rownames(lfc_state) = lfc_state$Function
lfc_state$Function = NULL

lfc_state = mutate(lfc_state, lfc = exp_state$res$`lfc_stateTexas`)
lfc_state = mutate(lfc_state, q_val = exp_state$res$`q_stateTexas`)

# See significant taxa
sig_func_state = filter(lfc_state, q_val < 0.05)

print(sig_func_state)

```


```{r GIFT_sig_state, fig.height=5, fig.width=10, fig.fullwidth=TRUE}
#Match significant functions to table
GIFTs_elements_community_no0_sig = GIFTs_elements_community_no0[, which(match(colnames(GIFTs_elements_community_no0), rownames(sig_func_state))>0)]
...
as.data.frame(GIFTs_elements_community_no0_sig) %>%
    ...
    ggplot(aes(x=sample,y=trait,fill=gift)) +
        ...
        scale_fill_gradientn(colours=rev(c("#2066a8", "#ededed"))) + 
        #facet_nested(~ species + sample_type,  scales="free", space="free") + 
        facet_grid(functionid ~ state, scales="free",space="free") +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
              strip.text.y = element_text(angle = 0), axis.text.y = element_text(angle=45, size=5)) + 
        labs(y="Functional Attribute",x="Samples",fill="GIFT")
