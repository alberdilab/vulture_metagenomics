...
## Element Level
```{r element_level_gift fig.height=10, fig.width=10, fig.fullwidth=TRUE}
...
# Example metadata with sample IDs and a grouping variable
# metadata <- data.frame(sample_id = colnames(otu_table), group = c("A", "A", "B", "B", ...))
...
# View results
lfc_df <- cbind(exp$res$`lfc_sample_typeStomach contents`)

#Identify differentially expressed genes
lfc_df <- as.data.frame(exp$res$'lfc_sample_typeStomach contents') %>%
  tibble::rownames_to_column(var = "taxon")


# See significant taxa
sig_taxa <- lfc_df %>%
  dplyr::filter(`q_sample_typeStomach contents` < 0.05)
