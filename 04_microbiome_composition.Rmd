...
adonis2(beta_q0n$S ~ state, 
        ...
        tt()
```
### Neutral Diversity (Species, Colon Fixed)
``` {r neutral_div_colon_species, warning=FALSE, comments="", message=FALSE}
group_neutral = metadata_colon[na.omit(match(metadata_colon$sample, names(beta_q1n_colon$S))),]

beta_q1n_colon$S %>%
  vegan::metaMDS(., trymax = 500, k = 2, trace=0) %>%
  vegan::scores() %>%
  as_tibble(., rownames = "sample") %>%
  dplyr::left_join(metadata_colon, by = join_by(sample == sample)) %>%
  group_by(species) %>%
  mutate(x_cen = mean(NMDS1, na.rm = TRUE)) %>%
  mutate(y_cen = mean(NMDS2, na.rm = TRUE)) %>%
  ungroup() %>%
...
betadisper(beta_q1n_colon$S, group_neutral$species) %>% permutest(., pairwise = TRUE) 

adonis2(beta_q1n_colon$S ~ species, 
        data = group_neutral %>% arrange(match(sample,labels(beta_q1n_colon$S))), 
        permutations = 999) %>%
        broom::tidy() %>%
        tt()
...
### Neutral Diversity (Species, Stomach Fixed)
``` {r neutral_div_stomach_species, warning=FALSE, comments="", message=FALSE}
group_neutral = metadata_stomach[na.omit(match(metadata_stomach$sample, names(beta_q1n_stomach$S))),]

beta_q1n_stomach$S %>%
  vegan::metaMDS(., trymax = 500, k = 2, trace=0) %>%
  vegan::scores() %>%
  as_tibble(., rownames = "sample") %>%
  dplyr::left_join(metadata_stomach, by = join_by(sample == sample)) %>%
  group_by(species) %>%
  mutate(x_cen = mean(NMDS1, na.rm = TRUE)) %>%
  mutate(y_cen = mean(NMDS2, na.rm = TRUE)) %>%
  ungroup() %>%
...
betadisper(beta_q1n_stomach$S, group_neutral$species) %>% permutest(., pairwise = TRUE) 

adonis2(beta_q1n_stomach$S ~ species, 
        data = group_neutral %>% arrange(match(sample,labels(beta_q1n_stomach$S))), 
        permutations = 999) %>%
        broom::tidy() %>%
        tt()
...
### Neutral Diversity (Sample Type, Cathartes aura Fixed)
``` {r neutral_div_caura_sampletype, warning=FALSE, comments="", message=FALSE}
...
beta_q1n_caura$S %>%
  ...
  vegan::scores() %>%
  as_tibble(., rownames = "sample") %>%
  dplyr::left_join(metadata_caura, by = join_by(sample == sample)) %>%
...
betadisper(beta_q1n_caura$S, group_neutral$sample_type) %>% permutest(., pairwise = TRUE) 

adonis2(beta_q1n_caura$S ~ sample_type, 
        data = group_neutral %>% arrange(match(sample,labels(beta_q1n_caura$S))), 
        permutations = 999) %>%
        broom::tidy() %>%
        tt()
...
### Neutral Diversity (Sample Type, Coragyps atratus Fixed)
``` {r neutral_div_ctratus_sampletype, warning=FALSE, comments="", message=FALSE}
group_neutral = metadata_catratus[na.omit(match(metadata_catratus$sample, names(beta_q1n_catratus$S))),]

beta_q1n_catratus$S %>%
  vegan::metaMDS(., trymax = 500, k = 2, trace=0) %>%
  vegan::scores() %>%
  as_tibble(., rownames = "sample") %>%
  dplyr::left_join(metadata_catratus, by = join_by(sample == sample)) %>%
  group_by(sample_type) %>%
  mutate(x_cen = mean(NMDS1, na.rm = TRUE)) %>%
  mutate(y_cen = mean(NMDS2, na.rm = TRUE)) %>%
  ungroup() %>%
...
betadisper(beta_q1n_catratus$S, group_neutral$sample_type) %>% permutest(., pairwise = TRUE) 

adonis2(beta_q1n_catratus$S ~ sample_type, 
        data = group_neutral %>% arrange(match(sample,labels(beta_q1n_catratus$S))), 
        permutations = 999) %>%
        broom::tidy() %>%
        tt()
...
## Function Matrix of sample type (Coragyps atratus fixed)
```{r Function_Distance_Matrix_catratus_sample }
GIFTs_catratus = GIFTs_elements_community_no0[which(rownames(GIFTs_elements_community_no0) %in% metadata_catratus$sample),]

dist_func = vegdist(GIFTs_catratus, method ="bray" )

group_neutral = metadata_catratus[na.omit(match(metadata_catratus$sample, names(beta_q1n_catratus$S))),]

adonis2(dist_func ~ group_neutral$sample_type, 
        data = as.data.frame(GIFTs_catratus) %>% arrange(match(group_neutral$sample,labels(beta_q1n_catratus$S))), 
        permutations = 999) %>%
        broom::tidy() %>%
        tt()
betadisper(dist_func, group_neutral$sample_type) %>% permutest()
...
```
## Function Matrix of species (Colon Contents fixed)
```{r Function_Distance_Matrix_colon_species }
GIFTs_colon = GIFTs_elements_community_no0[which(rownames(GIFTs_elements_community_no0) %in% metadata_colon$sample),]

dist_func = vegdist(GIFTs_colon, method ="bray" )

group_neutral = metadata_colon[na.omit(match(metadata_colon$sample, names(beta_q1n_colon$S))),]

adonis2(dist_func ~ group_neutral$species, 
        data = as.data.frame(GIFTs_colon) %>% arrange(match(group_neutral$sample,labels(beta_q1n_colon$S))), 
        permutations = 999) %>%
        broom::tidy() %>%
        tt()
betadisper(dist_func, group_neutral$species) %>% permutest()
...
```
## Function Matrix of species (Stomach Contents fixed)
```{r Function_Distance_Matrix_stomach_species }
GIFTs_stomach = GIFTs_elements_community_no0[which(rownames(GIFTs_elements_community_no0) %in% metadata_stomach$sample),]

dist_func = vegdist(GIFTs_stomach, method ="bray" )

group_neutral = metadata_stomach[na.omit(match(metadata_stomach$sample, names(beta_q1n_stomach$S))),]

adonis2(dist_func ~ group_neutral$species, 
        data = as.data.frame(GIFTs_stomach) %>% arrange(match(group_neutral$sample,labels(beta_q1n_stomach$S))), 
        permutations = 999) %>%
        broom::tidy() %>%
        tt()
betadisper(dist_func, group_neutral$species) %>% permutest()
...
```
### Differential Abundance of Bacteria by Sample Type (Cathartes aura fixed)
```{r ancom_caura_sampletype, warning=FALSE}
phylo_metadata_caura = metadata_caura
phylo_metadata_caura = data.frame(phylo_metadata_caura)
rownames(phylo_metadata_caura) = phylo_metadata_caura[,1]
phylo_metadata_caura = phylo_metadata_caura[,-1]
phylo_metadata_caura = sample_data(phylo_metadata_caura)

#phylo_var_caura <- apply(genome_counts_caura[-1], 1, var)
#genome_counts_caura_filt <- genome_counts_caura[phylo_var_caura > 0, ]

#prevalence <- rowSums(genome_counts_caura_filt > 0) / ncol(genome_counts_caura_filt)
#genome_counts_caura_filt <- genome_counts_caura_filt[prevalence > 0.1, ]

phylo_mag_caura = otu_table(genome_counts_caura[-1], taxa_are_rows=TRUE)

mag_table = phyloseq(phylo_mag_caura, phylo_metadata_caura)
...
phylo_metadata = sample_metadata
...
phylo_mag = otu_table(genome_counts[-1], taxa_are_rows=TRUE)
...
genome_counts_filt = column_to_rownames(genome_counts_filt, var = "genome")

phylo_mag = otu_table(genome_counts_filt, taxa_are_rows=TRUE)
...
```
### Volcano Plot (GI Location)
```{r community_elem_volcano, comment="", message=FALSE, warning=FALSE, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
sig_wilcox_sample$negLogP = -log10(sig_wilcox_sample$p_adjust)

ggplot(sig_wilcox_sample, aes(x = mean_difference, y = negLogP)) +
  geom_point(aes(color = negLogP > 1.3), alpha = 0.7) +  # Highlight significant points
...
```{r community_elem_volcano, comment="", message=FALSE, warning=FALSE, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
sig_wilcox_species$negLogP = -log10(sig_wilcox_species$p_adjust)

ggplot(sig_wilcox_species, aes(x = mean_difference, y = negLogP)) +
  geom_point(aes(color = negLogP > 1.3), alpha = 0.7) +  # Highlight significant points
  geom_text_repel(aes(label = trait_name), size = 3, max.overlaps = 10) +  # Avoid too many overlapping labels
  theme_minimal() +
  labs(x = "GIFT Difference",
